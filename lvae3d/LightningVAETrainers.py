...
from lvae3d.models.ResNetVAE_3D import ResNet18_3DVAE, ResNet18v2_3DVAE, ResNet34_3DVAE
...
class ResNet18v2VAE_alpha(L.LightningModule):
    def __init__(self, metadata, loss_func1=nn.BCEWithLogitsLoss(), loss_func2=SpectralLoss3D):
        """Lightning trainer for ResNet18v2_3DVAE. Loss is computed as a weighted sum between two loss functions, such
        that `loss = alpha * loss_func1 + (1 - alpha) * loss_func2`.
        ...
        self.metadata = metadata
        if self.metadata.metadata_dict['parallel'] is True:
            self.vae = nn.DataParallel(ResNet18v2_3DVAE(self.metadata.metadata_dict['latent_dim'],
                                                      self.metadata.metadata_dict['n_channels']))
        else:
            self.vae = ResNet18v2_3DVAE(self.metadata.metadata_dict['latent_dim'],
                                        self.metadata.metadata_dict['n_channels'])
        self.loss_func1 = loss_func1
        self.loss_func2 = loss_func2
        self.train_loss = []
...
class ResNet34VAE_alpha(L.LightningModule):
    def __init__(self, metadata, loss_func1=nn.BCEWithLogitsLoss(), loss_func2=SpectralLoss3D):
        ...
        self.metadata = metadata
        if self.metadata.metadata_dict['parallel'] is True:
            self.vae = nn.DataParallel(ResNet34_3DVAE(self.metadata.metadata_dict['latent_dim'],
                                                      self.metadata.metadata_dict['n_channels']))
        else:
            self.vae = ResNet34_3DVAE(self.metadata.metadata_dict['latent_dim'],
                                      self.metadata.metadata_dict['n_channels'])
        self.loss_func1 = loss_func1
        self.loss_func2 = loss_func2
        self.train_loss = []
...
class ResNet18v2VAE_beta(L.LightningModule):
    def __init__(self, metadata, loss_func=nn.MSELoss()):
        """Lightning trainer for ResNet18v2_3DVAE. Loss is computed as a weighted sum between two loss functions, such
        that `loss = loss_func + beta * KLDivergence`.
...
class ResNet34VAE_beta(L.LightningModule):
    def __init__(self, metadata, loss_func=nn.MSELoss()):
        """Lightning trainer for ResNet34_3DVAE. Loss is computed as a weighted sum between two loss functions, such
        that `loss = loss_func + beta * KLDivergence`.
...
class ResNet18VAE_alphabeta(L.LightningModule):
    def __init__(self, metadata, loss_func1=nn.BCEWithLogitsLoss(), loss_func2=SpectralLoss3D):
        """Lightning trainer for ResNet34_3DVAE. Loss is computed as a weighted sum between two loss functions, such
